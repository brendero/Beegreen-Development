    ...
    var App = {
        ...
        "updateUI": function() {
            if( this._widthHandlebarsAndLoDash) {
                this.updateUILecturers('feed-container');
            } else {
        ...
        "updateUIOldSchoolLecturers": function() {
            if(this._applicationDbContext.getTinderizeLecturersByUserId(this._activeUser.Id) != null) {
                ...
                for(var i=0;i<lecturers.length;i++) {
                    ...
                    tempStr += '<a href="profile-page.html">';
                    tempStr += '<div class="items-container>';
                    tempStr += '<img class="square-picture" src="'+ lecturer.Picture +'">';
                    tempStr += '</div>';
                    tempStr += '<div class="text-content">';
                    tempStr += '<h4 class="bitter friend-name">' + lecturer.FirstName + lecturer.SurName +'</h4>';
                    tempStr += '<span class="raleway>'+ '</span><br>';
                    tempStr += '<span class="raleway>'+ '</span>';
                    tempStr += '</div>';
                    tempStr += '</div>';
                    tempStr += '</a>';
                };

                document.querySelector('.feed-container').innerHTML = tempStr;
        ...
        "updateUILecturers": function(hbsTmplName, hbsTmplId) {
			...
			document.querySelector('.feed-container').innerHTML = this._hbsCache[hbsTmplName](this._applicationDbContext.getTinderizeLecturersByUserId(this._activeUser.Id));// Write compiled content to the appropriate container
        ...
        "registerLecturerEventListeners": function() {
            ...
            var lecturerElements = document.querySelectorAll('.items-container');
        ...
        "addTinderizeLecturer": function(lecturerId, tinderize) {
            ...
            if(tinderizeLecturerAdded != null) {
                var lecturerElement = document.querySelector(`.items-container[data-id="${lecturerId}"]`);
